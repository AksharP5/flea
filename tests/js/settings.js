...
function runRows(check) {
    ...
    check("the current menu controls include Permissions and the retained hints preference",
          ...
          "cut,copy,paste,duplicate,rename,trash,delete,openwith,openTerminal,moveto,copyto,properties,permissions,copypath,shelf,compress,extract,convert,taildrop,dropbox,sharelink,placeMenu,keyHints")
...
function runPresets(check) {
    ...
    check("SettingsMenus contains exactly 22 action switches", menuRows.filter(function (r) { return r.kind === "check" && r.id !== "keyHints" }).length, 22)
